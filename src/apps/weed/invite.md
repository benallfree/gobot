...
You can run the latest version of `weed` anywhere npm/npx is available:

```bash
npx gobot weed --help
```

The package is [https://npmjs.org/gobot-weed](https://npmjs.org/gobot-weed).

## What's next

- **Test & Feedback.** Please test and review as you are able. I have tagged things as alpha for now, but I will promote it to stable after a brief testing period and back-publish all 244 versions of SeaweedFS.
...
`gobot-weed` versions mirror `weed` versions. Gobot has indexed 244 releases of `weed`:
...
| 3.18.0  | arm64/x64/arm      | arm64     | arm64/x64/arm      | x64      |
