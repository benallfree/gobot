...
You can run the latest version of `minio` anywhere npm/npx is available:

```bash
npx gobot minio --help
```

The package is [https://npmjs.org/gobot-minio](https://npmjs.org/gobot-minio).

## What's next

- **Test & Feedback.** Please test and review as you are able. I have tagged things as alpha for now, but I will promote it to stable after a brief testing period and back-publish all 468 versions of Minio.
...
`gobot-minio` versions mirror `minio` versions. Gobot has indexed 468 releases of `minio`:
