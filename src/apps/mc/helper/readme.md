...
## `mc` as an npm package

This is an optional [Gobot](https://github.com/benallfree/gobot) helper used when you want [Mc](https://min.io) to be a dependency in your project.

With it, you can ensure that `mc` is always installed and available to your application, in the specific version (or range) you require.

`mc` can also be surfaced as a CLI alias by installing this package globally.

## Usage

**Basic version locking**

Install `gobot` and `gobot-mc`:

```bash
npm i gobot gobot-mc
```
...
/**
 * With `gobot-mc present, Gobot will default
 * to the `mc` version corresponding to
 * `package.version`
 */
...
View the [Mc sample project](https://github.com/benallfree/gobot/tree/v1.0.0-alpha.14/src/apps/mc/sample-project) on github.
