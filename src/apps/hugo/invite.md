...
You can run the latest version of `hugo` anywhere npm/npx is available:

```bash
npx gobot hugo --help
```

The package is [https://npmjs.org/gobot-hugo](https://npmjs.org/gobot-hugo).

## What's next

- **Test & Feedback.** Please test and review as you are able. I have tagged things as alpha for now, but I will promote it to stable after a brief testing period and back-publish all 260 versions of Hugo.
...
`gobot-hugo` versions mirror `hugo` versions. Gobot has indexed 260 releases of `hugo`:
