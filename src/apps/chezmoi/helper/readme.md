...
const bot = await gobot(`chezmoi`, { version: `2.47.2` })
bot.run([`--version`])
...
View the [chezmoi sample project](https://github.com/benallfree/gobot/tree/v1.0.0-alpha.34/src/apps/chezmoi/sample-project) on github.
...
`gobot-chezmoi` versions mirror `chezmoi` versions. Gobot knows about 196 releases of `chezmoi`:
