...
# Caddy plugin for Gobot

_Install, run, and manage `caddy` as an npm package with CLI and API interfaces._

## What is Caddy?

![Gobot](https://raw.githubusercontent.com/benallfree/gobot/feat/plugin-placeholders/src/plugins/caddy/logo-50x.png)

Fast and extensible multi-platform HTTP/1-2-3 web server with automatic HTTPS

## Quickstart

This plugin provides [Caddy](https://caddyserver.com/) support to [Gobot](https://github.com/benallfree/gobot). It is built in to Gobot.
...
Or use the `gobot-caddy` helper package to get a CLI bin alias (the main Gobot package omits specific bin aliases to avoid CLI name pollution):
...
gobot --g-debug caddy --help

# Run a specific  version
gobot --g-version="2.7.*" caddy --help
...
Inside your project, use the Gobot helper to add support for `caddy`:

```bash
npx gobot init caddy
```

The above command will find the latest version of Caddy and modify your `package.json` as follows:
...
## Notes about `Caddy` support

This section is empty, which means this plugin has no special notes.

## Sample project

View the [Caddy sample project](https://github.com/benallfree/gobot/tree/feat/plugin-placeholders/src/plugins/caddy/sample-project) on github.
